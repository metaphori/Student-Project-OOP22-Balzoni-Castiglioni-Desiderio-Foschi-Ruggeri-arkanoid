...
public class GameViewImpl extends JPanel implements KeyListener,ActionListener, GameView{
    ...
    public void paintComponent(Graphics g){
       ...
       observer.getList().entrySet().stream().forEach(x->{
            ...
            x.getKey().getY() == observer.getRowC(5d) ? Color.CYAN : Color.GREEN);
    ...
    public void keyPressed(KeyEvent arg0) {
        // TODO Auto-generated method stub
        Double x=observer.getPad().getX();
        Double y=observer.getPad().getY();
        ...
        if(arg0.getKeyCode() == KeyEvent.VK_LEFT) {
            if(x<=0){
                observer.changePosPad(new Pair<Double, Double>(0d,y));
            } else {
                moveLeft();
            }
        }
    ...
    private void moveLeft() {
        ...
        if(observer.getPad().getX()-20>=0){
            ...
            observer.changePosPad(new Pair<>(0d,observer.getPad().getY()));
        }
    }
