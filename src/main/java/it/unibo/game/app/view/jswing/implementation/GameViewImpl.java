...
public class GameViewImpl extends JPanel implements KeyListener, ActionListener, GameView {
	...
	public void paintComponent(Graphics g) {
		...
		observer.getList().entrySet().stream().forEach(x -> {
											...
											: x.getKey().getY().intValue() == observer.getRowC(2d).intValue() ? Color.YELLOW
													: x.getKey().getY().intValue() == observer.getRowC(3d).intValue() ? Color.MAGENTA
															: x.getKey().getY().intValue() == observer.getRowC(4d).intValue() ? Color.ORANGE
																	: x.getKey().getY().intValue() == observer.getRowC(5d).intValue() ? Color.CYAN
																			: Color.GREEN);
			...
			var rec = new Rectangle2D.Double(x.getKey().getX(), x.getKey().getY(), observer.getDimensionBrick().getY(),
					observer.getDimensionBrick().getX());
			g2d.fill(rec);
		...
		g2d.fill(new Ellipse2D.Double(observer.getBall().getX(), observer.getBall().getY(), observer.getRBall(),
				observer.getRBall()));

		observer.getSurprise().stream().forEach(x -> {
			g2d.setColor(Color.RED);
			g2d.fill(new Ellipse2D.Double(x.getX(), x.getY(), observer.getRBall(), observer.getRBall()));
		});

		g2d.setColor(Color.BLACK);
		g2d.fill(new Rectangle2D.Double(observer.getPadPos().getX(), observer.getPadPos().getY(), observer.getPadWight(),
				observer.getPadHeight()));
